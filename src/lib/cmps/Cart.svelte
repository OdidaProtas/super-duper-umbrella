<script lang="ts">
  ...
  const checkoutHandler = () => {
    ...
    if (!flag3) {
      checkout.classList.add("translate-x-screen");
      ...
      setTimeout(function () {
        checkout.classList.remove("translate-x-screen");
...
<div class="">
  <button on:click={() => checkoutHandler(false)}>Open Modal</button>
</div>
<div
  class="w-full h-full hidden bg-black dark:bg-black bg-opacity-90 top-0 overflow-y-auto overflow-x-hidden fixed sticky-0"
  id="chec-div"
>
  <div
    ...
    <div class="flex items-end lg:flex-row flex-col justify-end" id="cart">
      ...
      >
        <button
          ...
          on:click={() => checkoutHandler(false)}
        >
          <img
            class="dark:hidden"
      ...
      <div class="lg:w-96 md:w-8/12 w-full bg-gray-100 dark:bg-black h-full">
        <div
